		...
		if(item != undefined){
			...
			if(JSON.parse(item).type_module == "tables"){
				...
				JSON.parse(item).columns.forEach((e,i)=>{
					...
					var typeColumn = ["text","textarea","int","double","image","video","file","boolean","select","array","object","json","date","time","datetime","timestamp","code","link","color","money","password","email","relations"];
					var selectColumn = [];
